...
仪表盘的 **标的** 页把视角从"账户"翻转成"标的"——按 ticker 聚合所有账户在它上面的交易。

**列表视图**：每行一个 ticker，显示账户数 / 交易次数 / 已实现盈亏 / 最近交易日期。四列表头都可点击排序，配合搜索框可快速定位。

**详情视图**（点任一 ticker 进入）：

- **价格图 + 多账户交易点位**：每个交易过该 ticker 的账户用一个独立颜色，买卖箭头标在 K 线上，hover 显示具体账户、方向、数量、成交价
- **公司简介卡片**（右侧）：公司全称、GICS sector / industry、2-3 句业务描述、下次财报日期、官网。中英双语自动切换（Google Translate 缓存到本地，不重复请求）
- **同业公司**：基于 GICS industry 自动列出同行业其他票（如 NVDA → AMD、AVGO、MU、INTC、QCOM...），点击直接跳转
- **每账户盈亏表**：每个账户的策略名、交易次数、已实现 / 浮动 / 总盈亏、收益率、当前持仓；点击行展开 FIFO 逐笔账本，逐笔 PnL + 持仓变化

数据来源：yfinance（公司信息 / 同业分类）+ Russell 1000 universe（同业列表）。
